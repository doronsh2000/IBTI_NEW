...
/*
 if(   (last_green_bar_open > marketData[0].close[marketData[0].close.length-1]) && take_profit_signal=='BUY'  ){
         ...
         console.log('exiting LONG!!- GBP');
          fs.appendFileSync('./signals.txt','exiting LONG!!- GBP' + new Date() + ' \n');
...
var event=new RedisEvent('localhost',['forex_buy_and_sell']);
 ...
 //if( (  last_red_bar_open < marketData[0].close[marketData[0].close.length-1]) && take_profit_signal=='SELL' ){
  ...
  if( (  last_red_bar_open < marketData[0].close[marketData[0].close.length-1]) && take_profit_signal=='SELL' ){
          ...
          console.log('exiting SHORT!! - GBP');
          fs.appendFileSync('./signals.txt','exiting SHORT!! - GBP' + new Date() + '\n');
var event=new RedisEvent('localhost',['forex_buy_and_sell']);
        event.on('ready',function(){
           event.pub('forex_buy_and_sell:app',{
...
talib.execute({
    ...
    console.log('current GBP high is: ' +  marketData[0].high[marketData[0].high.length-1]);
    console.log('current GBP low is: ' +  marketData[0].low[marketData[0].low.length-1]);
